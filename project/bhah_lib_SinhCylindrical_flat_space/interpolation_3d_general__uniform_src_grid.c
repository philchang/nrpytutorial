...
// Thiago says: all pragma statements have been commented out, because they make the code slower for a single interpolation.
...
// /**
//  **********************************************************************************************
//  ********************************   ADAPTED 2D INTERPOLATOR    ********************************
//  * Thiago says: ignoring phi coordinate for interpolation
//  **********************************************************************************************
// */
...
// /**
//  * 2D Lagrange interpolation in x0 & x2, with x1 fixed at the central ghost index.
//  * Unused x1 arrays and variables have been removed to eliminate warnings.
//  */
// int interpolation_3d_general__uniform_src_grid(
//   const int    N_interp_GHOSTS,
//   const REAL   src_dxx0,
//   const REAL   src_dxx1,    // ignored internally
//   const REAL   src_dxx2,
//   const int    src_Nxx_plus_2NGHOSTS0,
//   const int    src_Nxx_plus_2NGHOSTS1,  // only for pointer strides
//   const int    src_Nxx_plus_2NGHOSTS2,
//   const int    NUM_INTERP_GFS,
//   REAL *restrict     src_x0x1x2[3],
//   const REAL *restrict src_gf_ptrs[NUM_INTERP_GFS],
//   const int    num_dst_pts,
//   const REAL   dst_x0x1x2[][3],
//   REAL *restrict dst_data[NUM_INTERP_GFS]
// ) {
//   const int NinterpGHOSTS = N_interp_GHOSTS;
...
//   // 2D normalization over x0 & x2 only
//   const REAL src_invdxx02_INTERP_ORDERm1 =
//     pow(src_dxx0 * src_dxx2, -(INTERP_ORDER - 1));
...
//   // Ensure ORDER fits x0 & x2
//   if (INTERP_ORDER > src_Nxx_plus_2NGHOSTS0 ||
//       INTERP_ORDER > src_Nxx_plus_2NGHOSTS2)
//     return INTERP3D_GENERAL_INTERP_ORDER_GT_NXX123;

//   // Precompute inverse denominators (unchanged)
//   REAL inv_denom[INTERP_ORDER];
//   for (int i = 0; i < INTERP_ORDER; i++) {
//     REAL denom = 1.0;
//     for (int j = 0; j < i; j++)     denom *= (REAL)(i - j);
//     for (int j = i + 1; j < INTERP_ORDER; j++) denom *= (REAL)(i - j);
//     inv_denom[i] = 1.0 / denom;
//   }

//   // Fix x1 at the center of ghost zones
//   const int fixed_idx_x1 = NinterpGHOSTS;

//   const REAL xx0_min = src_x0x1x2[0][0];
//   const REAL xx2_min = src_x0x1x2[2][0];
...
//     int idx0 = (int)((x0_dst - xx0_min) * src_invdxx0 + 0.5);
//     int idx2 = (int)((x2_dst - xx2_min) * src_invdxx2 + 0.5);

//     // Bounds check for x0 & x2
//     if ((idx0 - NinterpGHOSTS < 0) ||
//         (idx0 + NinterpGHOSTS >= src_Nxx_plus_2NGHOSTS0) ||
//         (idx2 - NinterpGHOSTS < 0) ||
//         (idx2 + NinterpGHOSTS >= src_Nxx_plus_2NGHOSTS2)) {
// // #pragma omp critical
//       {
//         error_flag = INTERP3D_GENERAL_HORIZON_OUT_OF_BOUNDS;
//         idx0 = idx2 = NinterpGHOSTS;
//       }
//       continue;
//     }

//     const int base0 = idx0 - NinterpGHOSTS;
//     const int base2 = idx2 - NinterpGHOSTS;

//     // Differences in x0 & x2
//     REAL diffs_x0[INTERP_ORDER], diffs_x2[INTERP_ORDER];
// // #pragma omp simd
//     for (int j = 0; j < INTERP_ORDER; j++) {
//       diffs_x0[j] = x0_dst - src_x0x1x2[0][base0 + j];
//       diffs_x2[j] = x2_dst - src_x0x1x2[2][base2 + j];
//     }

//     // 1D basis coefficients in x0 & x2
//     REAL coeff_x0[INTERP_ORDER], coeff_x2[INTERP_ORDER];
// // #pragma omp simd
//     for (int i = 0; i < INTERP_ORDER; i++) {
//       REAL numer_x0 = 1.0, numer_x2 = 1.0;
//       for (int j = 0; j < i; j++) {
//         numer_x0 *= diffs_x0[j];
//         numer_x2 *= diffs_x2[j];
//       }
...
//         numer_x2 *= diffs_x2[j];
//       }
//       coeff_x0[i] = numer_x0 * inv_denom[i];
//       coeff_x2[i] = numer_x2 * inv_denom[i];
//     }

//     // Combine into a 2D coefficient table [ix2][ix0]
//     REAL coeff_2d_x2_x0[INTERP_ORDER][INTERP_ORDER];
//     for (int i2 = 0; i2 < INTERP_ORDER; i2++) {
//       for (int i0 = 0; i0 < INTERP_ORDER; i0++) {
//         coeff_2d_x2_x0[i2][i0] = coeff_x0[i0] * coeff_x2[i2];
//       }
//     }

//     // Interpolate each grid function
//     for (int gf = 0; gf < NUM_INTERP_GFS; gf++) {
//       REAL sum = 0.0;
//       for (int i2 = 0; i2 < INTERP_ORDER; i2++) {
//         int i0 = 0;
...
//         // Flat offset uses fixed x1 slice
//         const int base_offset =
//           base0
//           + src_Nxx_plus_2NGHOSTS0 * (
//               fixed_idx_x1
//             + src_Nxx_plus_2NGHOSTS1 * (base2 + i2)
//           );

//         // SIMD inner loop
//         for (; i0 <= INTERP_ORDER - simd_width; i0 += simd_width) {
//           const int idx = base_offset + i0;
//           REAL_SIMD_ARRAY vsrc   = ReadSIMD(&src_gf_ptrs[gf][idx]);
//           REAL_SIMD_ARRAY vcoeff = ReadSIMD(&coeff_2d_x2_x0[i2][i0]);
//           vec_sum = FusedMulAddSIMD(vsrc, vcoeff, vec_sum);
//         }
//         sum += HorizAddSIMD(vec_sum);

//         // Remainder
//         for (; i0 < INTERP_ORDER; i0++) {
//           sum += src_gf_ptrs[gf][base_offset + i0]
//                * coeff_2d_x2_x0[i2][i0];
//         }
//       }

//       // Store with 2D normalization
//       dst_data[gf][dst_pt] = sum * src_invdxx02_INTERP_ORDERm1;
//     }
//   }
