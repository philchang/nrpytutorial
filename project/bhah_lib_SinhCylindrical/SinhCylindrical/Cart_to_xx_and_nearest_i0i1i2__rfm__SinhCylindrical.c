...
#include "../set_CodeParameters.h"
  ...
  Cartz -= Cart_originz;
  {
     ...
     *  "[xx[0] = SINHWRHO*asinh(sqrt(Cartx**2 + Carty**2)*sinh(1/SINHWRHO)/AMPLRHO)]"
     *  "[xx[1] = atan2(Carty, Cartx)]"
     *  "[xx[2] = SINHWZ*asinh(Cartz*sinh(1/SINHWZ)/AMPLZ)]"
     */
    xx[0] = SINHWRHO * asinh(sqrt(((Cartx) * (Cartx)) + ((Carty) * (Carty))) * sinh((1.0 / (SINHWRHO))) / AMPLRHO);
    xx[1] = atan2(Carty, Cartx);
    xx[2] = SINHWZ * asinh(Cartz * sinh((1.0 / (SINHWZ))) / AMPLZ);
