...
static void ghl_hybrid_get_K_and_Gamma(const TOVola_data_struct *TOVdata, REAL rho_baryon, REAL *restrict K, REAL *restrict Gamma) {
...
static void ghl_hybrid_compute_P_cold_and_eps_cold(const TOVola_data_struct *TOVdata, REAL rho_baryon, REAL *restrict P, REAL *restrict eps) {
...
int TOVola_ODE(REAL r_Schw, const REAL y[], REAL dydr_Schw[], void *params) {
...

  {
     ...
     *  "[dydr_Schw[TOVOLA_PRESSURE] = -(rho_energy + y[TOVOLA_PRESSURE])*(8*M_PI*r_Schw**2*y[TOVOLA_PRESSURE] + 2*y[TOVOLA_MASS]/r_Schw)/(2*r_Schw*(1
     * - 2*y[TOVOLA_MASS]/r_Schw))]"
     *  "[dydr_Schw[TOVOLA_NU] = (8*M_PI*r_Schw**2*y[TOVOLA_PRESSURE] + 2*y[TOVOLA_MASS]/r_Schw)/(r_Schw*(1 - 2*y[TOVOLA_MASS]/r_Schw))]"
     *  "[dydr_Schw[TOVOLA_MASS] = 4*M_PI*r_Schw**2*rho_energy]"
     *  "[dydr_Schw[TOVOLA_R_ISO] = y[TOVOLA_R_ISO]/(r_Schw*sqrt(1 - 2*y[TOVOLA_MASS]/r_Schw))]"
     */
    {
      const REAL tmp0 = (1.0 / (r_Schw));
      const REAL tmp3 = M_PI * ((r_Schw) * (r_Schw));
      const REAL tmp1 = 2 * tmp0 * y[TOVOLA_MASS];
      const REAL tmp2 = 1 - tmp1;
      const REAL tmp4 = tmp0 * (tmp1 + 8 * tmp3 * y[TOVOLA_PRESSURE]) / tmp2;
      dydr_Schw[TOVOLA_PRESSURE] = -1.0 / 2.0 * tmp4 * (rho_energy + y[TOVOLA_PRESSURE]);
      dydr_Schw[TOVOLA_NU] = tmp4;
      dydr_Schw[TOVOLA_MASS] = 4 * rho_energy * tmp3;
      dydr_Schw[TOVOLA_R_ISO] = tmp0 * y[TOVOLA_R_ISO] / sqrt(tmp2);
    }
  }
...
void extend_to_negative_r(REAL *restrict arr, const REAL parity, REAL *restrict tmp, const TOVola_data_struct *restrict TOVdata) {
  for (int i = 0; i < NEGATIVE_R_INTERP_BUFFER; i++)
    tmp[i] = parity * arr[NEGATIVE_R_INTERP_BUFFER - i - 1];
  for (int i = 0; i < TOVdata->numpoints_actually_saved; i++)
    tmp[i + NEGATIVE_R_INTERP_BUFFER] = arr[i];
  memcpy(arr, tmp, sizeof(REAL) * (TOVdata->numpoints_actually_saved + NEGATIVE_R_INTERP_BUFFER));
}

/**
 * Driver routine for TOV solve.
 */
