...
#include "../set_CodeParameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cylindrical (orig coord = SinhCylindrical):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cylindrical (orig coord = SinhCylindrical)
  /*
   *  Original SymPy expressions:
   *  "[Cart_to_xx0_inbounds = sqrt(Cartx**2 + Carty**2)]"
   *  "[Cart_to_xx1_inbounds = atan2(Carty, Cartx)]"
   *  "[Cart_to_xx2_inbounds = Cartz]"
   */
  {
    Cart_to_xx0_inbounds = sqrt(((Cartx) * (Cartx)) + ((Carty) * (Carty)));
    Cart_to_xx1_inbounds = atan2(Carty, Cartx);
    Cart_to_xx2_inbounds = Cartz;
  }
  ...
  {
    // xx_to_Cart for Coordinate SinhCylindrical):
    DOUBLE xx0 = xx[0][i0];
    DOUBLE xx1 = xx[1][i1];
    DOUBLE xx2 = xx[2][i2];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx = AMPLRHO*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))*cos(xx1)/(exp(1/SINHWRHO) - exp(-1/SINHWRHO))]"
     *  "[yCart_from_xx = AMPLRHO*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))*sin(xx1)/(exp(1/SINHWRHO) - exp(-1/SINHWRHO))]"
     *  "[zCart_from_xx = AMPLZ*(exp(xx2/SINHWZ) - exp(-xx2/SINHWZ))/(exp(1/SINHWZ) - exp(-1/SINHWZ))]"
     */
    ...
    xCart_from_xx = tmp2 * cos(xx1);
    yCart_from_xx = tmp2 * sin(xx1);
    zCart_from_xx = AMPLZ * (exp(tmp3 * xx2) - exp(-tmp3 * xx2)) / (exp(tmp3) - exp(-tmp3));
  }
  ...
  {
    // xx_to_Cart_inbounds for Coordinate SinhCylindrical):
    DOUBLE xx0 = xx[0][i0_inbounds];
    DOUBLE xx1 = xx[1][i1_inbounds];
    DOUBLE xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = AMPLRHO*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))*cos(xx1)/(exp(1/SINHWRHO) - exp(-1/SINHWRHO))]"
     *  "[yCart_from_xx_inbounds = AMPLRHO*(exp(xx0/SINHWRHO) - exp(-xx0/SINHWRHO))*sin(xx1)/(exp(1/SINHWRHO) - exp(-1/SINHWRHO))]"
     *  "[zCart_from_xx_inbounds = AMPLZ*(exp(xx2/SINHWZ) - exp(-xx2/SINHWZ))/(exp(1/SINHWZ) - exp(-1/SINHWZ))]"
     */
    const REAL tmp0 = (1.0 / (SINHWRHO));
    const REAL tmp3 = (1.0 / (SINHWZ));
    const REAL tmp2 = AMPLRHO * (exp(tmp0 * xx0) - exp(-tmp0 * xx0)) / (exp(tmp0) - exp(-tmp0));
    xCart_from_xx_inbounds = tmp2 * cos(xx1);
    yCart_from_xx_inbounds = tmp2 * sin(xx1);
    zCart_from_xx_inbounds = AMPLZ * (exp(tmp3 * xx2) - exp(-tmp3 * xx2)) / (exp(tmp3) - exp(-tmp3));
  }
...
#define EPS_REL 1e-8
  ...
  if (fabs((DOUBLE)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,
            "Error in SinhCylindrical coordinate system: Inner boundary point does not map to grid interior point: ( %.15e %.15e %.15e ) != ( %.15e "
...
#define EPS_REL 1e-8
  ...
  DOUBLE REAL_parity_array[10];
  {
    ...
    */
     ...
     *  "[REAL_parity_array[1] = sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds)]"
     *  "[REAL_parity_array[2] = sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds)]"
     *  "[REAL_parity_array[3] = 1]"
     *  "[REAL_parity_array[4] = (sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds))**2]"
     *  "[REAL_parity_array[5] = (sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds))**2]"
     *  "[REAL_parity_array[6] = sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds)]"
     *  "[REAL_parity_array[7] = (sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds))**2]"
     *  "[REAL_parity_array[8] = sin(xx1)*sin(xx1_inbounds) + cos(xx1)*cos(xx1_inbounds)]"
     *  "[REAL_parity_array[9] = 1]"
     */
    {
      const REAL tmp0 = sin(xx1) * sin(xx1_inbounds) + cos(xx1) * cos(xx1_inbounds);
      const REAL tmp1 = ((tmp0) * (tmp0));
      REAL_parity_array[0] = 1;
      REAL_parity_array[1] = tmp0;
      REAL_parity_array[2] = tmp0;
      REAL_parity_array[3] = 1;
      REAL_parity_array[4] = tmp1;
      REAL_parity_array[5] = tmp1;
      REAL_parity_array[6] = tmp0;
      REAL_parity_array[7] = tmp1;
      REAL_parity_array[8] = tmp0;
      REAL_parity_array[9] = 1;
    }
  }
